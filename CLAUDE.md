...
Search AnimeUnity's catalog instead of downloading a URL (`--search`, `anime_downloader.py` only; mutually exclusive with the positional `url` via an `argparse` mutually-exclusive group, one of the two is required). Exports every match to its own file under `Search/` (or `<custom_path>/Search/`) — a `.txt` with just the title for a movie, a `.csv` of every episode's season/number/title/link for a series — without downloading any video. `--start`/`--end`/`--episodes`/`--check`/`--parallel-downloads` don't apply here.
```bash
